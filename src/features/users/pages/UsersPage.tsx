...
export default function UsersPage() {
  ...
  const handleConfirmDelete = async (reason: string) => {
    ...
    try {
      await userService.deleteUser(deletingUser.id, reason);
      toast.success('Usuario enviado al Histórico exitosamente');
      triggerRefresh();
    } catch (error) {
      ...
      <div className="bg-white rounded-lg shadow-md p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-6">
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
