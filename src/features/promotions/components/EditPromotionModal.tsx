...
// Convierte el valor de fecha que puede venir como string ISO, Date o Timestamp
const toDateInputValue = (val: Date | string | undefined | null): string => {
  if (!val) return '';
...
// Normaliza los productos de la promoción: si vienen como string o como objeto
// expandido (en caso de que el backend devuelva populate), siempre devolvemos
// el shape { productId, minimumQuantity } esperado por el modelo.
const normalizeProducts = (
  raw: Promotion['products']
): NonNullable<Promotion['products']> => {
  if (!raw) return [];
  return raw.map(p => {
    const anyP = p as any;
    const id = typeof anyP.productId === 'string'
      ? anyP.productId
      : anyP.productId?._id || anyP.productId?.id;
    return {
      productId: String(id),
      minimumQuantity: anyP.minimumQuantity ?? 1,
    } as NonNullable<Promotion['products']>[number];
  });
};
...
export function EditPromotionModal({ promotion, onClose, onSubmit }: EditPromotionModalProps) {
  ...
  // Inicialización única del form con los datos de la promoción (sin bucles)
  const [formData, setFormData] = useState<Partial<Promotion>>(() => ({
    ...promotion,
    products: normalizeProducts(promotion.products),
    startDate: toDateInputValue(promotion.startDate as any) as any,
    endDate: toDateInputValue(promotion.endDate as any) as any,
    conditions: {
      minimumPurchase: promotion.conditions?.minimumPurchase ?? 0,
      maxUses: promotion.conditions?.maxUses ?? null,
      usedCount: promotion.conditions?.usedCount ?? 0,
    },
  }));
  ...
  return (
    <BaseModal isOpen={true} onClose={onClose} title="Editar Promoción" size="2xl" footer={footer}>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-3 p-3 rounded-xl border border-gray-100 bg-white">
              ...
              <Input
                ...
                min={(formData.startDate as string) || undefined}
