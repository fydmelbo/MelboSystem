...
export default function EditProductModal({ product, onClose, onSubmit }: EditProductModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const processedData = {
      ...
      purchasePrices: {
        unit: purchasePrices.unit ? Number(purchasePrices.unit) : 0,
        blister: purchasePrices.blister ? Number(purchasePrices.blister) : 0,
        box: purchasePrices.box ? Number(purchasePrices.box) : 0
      },
      stock: {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              >
                ...
                {/* Si el producto tiene una categoría que no está en la lista, mostrarla */}
                {formData.category && !categories.some(c => c.name === formData.category) && (
                  <option value={formData.category}>{formData.category} (no catalogada)</option>
                )}
              </select>
            </div>
            <div>
              ...
              >
                ...
                {formData.pharmaceuticalCompany && !pharmaCompanies.some(c => c.name === formData.pharmaceuticalCompany) && (
                  <option value={formData.pharmaceuticalCompany}>{formData.pharmaceuticalCompany} (no catalogada)</option>
                )}
              </select>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setPurchasePrices({ ...purchasePrices, unit: e.target.value })}
                  className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setPurchasePrices({ ...purchasePrices, blister: e.target.value })}
                  className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setPurchasePrices({ ...purchasePrices, box: e.target.value })}
                  className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                />
              </div>
            </div>
