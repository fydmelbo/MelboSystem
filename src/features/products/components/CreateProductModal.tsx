...
export default function CreateProductModal({ onClose, onSubmit }: CreateProductModalProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    purchasePrices: { unit: '', blister: '', box: '' },
    stock: { units: '', blisters: '', boxes: '', initial: '' },
    packaging: { unitsPerBlister: '', blistersPerBox: '', unitsPerBox: '', description: '' },
  ...
  const handleBoxesChange = (value: string) => {
    ...
    const result = autoFillFromBoxes(numBoxes, getPackaging(), getSellOptions());
    setFormData(prev => ({
      ...prev,
      stock: {
        ...prev.stock,
  ...
  const handleBlistersChange = (value: string) => {
    ...
    const result = normalizeFromBlisters(numBlisters, getPackaging(), getSellOptions());
    setFormData(prev => ({
      ...prev,
      stock: {
        ...prev.stock,
  ...
  const handleUnitsChange = (value: string) => {
    ...
    const result = normalizeFromUnits(numUnits, getPackaging(), getSellOptions());
    setFormData(prev => ({
      ...prev,
      stock: {
        ...prev.stock,
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div>
                <SearchableSelect
                  ...
                  options={pharmaCompanies.map((comp) => ({ value: comp.name, label: comp.name }))}
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input type="text" value={formData.invoice} onChange={(e) => setFormData({ ...formData, invoice: e.target.value })} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" placeholder="Ej: FAC-2023-001" />
              </div>
              <div>
            ...
            <div className="mt-8 bg-blue-50 p-4 rounded-xl border border-blue-100">
              <h4 className="text-blue-800 font-medium mb-2">Resumen del Producto</h4>
  ...
  const footer = (
    <div className="flex items-center justify-between w-full">
      ...
      <Button
        ...
        disabled={isSubmitting}
      >
        {currentStep === STEP_LABELS.length - 1 ? 'Guardar Producto' : 'Siguiente'}
      </Button>
    </div>
  );
